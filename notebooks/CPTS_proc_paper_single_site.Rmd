...
if (curr_skipcells == FALSE){
    ...
    data_allsite_file <- 'C:\\Users\\mhollaway\\Documents\\DSNE_code\\DSNE_devel\\Data\\CPTS_data\\CPTS_DATA_PREPROC.csv'
...
```{r}
if (curr_skipcells == FALSE){
  kable(data.frame(Site=curr_site,N_reps=curr_nreps,Minimum_seg_len=curr_minseg,
...
cpts_confint <- function(x_series,x_series_orig,x_cpts,N_reps,n_cpts,cpt_method,AR_order){
  ...
  cpt_boot <- function(seg_1,seg_2,cpt_method,AR_order){
      ...
      #Now estimate the new changepoint (fix at 1 using AMOC).
		  #Also need to define this base on chosen cpt method (trend method requires data pre-processing as before.
		  if (cpt_method == 'mean'){
		  ...
		  } else if (cpt_method == 'trend'){
		    trend_samp_proc <- c(seg_1_boot,seg_2_boot)
			n_trend         <- length(trend_samp_proc)
		    if (AR_order == 0){ 
			  cpt_samp_trend <- cbind(trend_samp_proc,rep(1,n_trend),1:n_trend)
			} else if (AR_order == 1){
			  cpt_samp_trend <- cbind(trend_samp_proc[-1],rep(1,n_trend-1),1:(n_trend-1),trend_samp_proc[-n_trend])
			} else if (AR_order == 2){
			  cpt_samp_trend <- cbind(trend_samp_proc[-c(1:2)],rep(1,n_trend-2),1:(n_trend-2),
			                          trend_samp_proc[2:(n_trend-1)],trend_samp_proc[1:(n_trend-2)])
			}
			cpt_samp <- cpt.reg(cpt_samp_trend,method='AMOC',penalty='MBIC')
		  }
   ...
   for (kk in 1:length(obs_pres_segs)){
      ...
      if (length(curr_seg_ids) > min_seg_proc){
        ...
        #Check to see if there are any changepoints - of there are append to the main array.
		    #If calculating cpts on trend remember to pass the non arima time series so cpts can be calculated as in envcpt package.
...
EVAL_out_file <- paste("C:\\Users\\mhollaway\\Documents\\TEST_DATA\\Shiny_out_test\\",curr_site,"_",curr_cptmethod,"_eval.xlsx",sep='')
...
writeData(wb_out, "MOD_CPT_SUMMARY",  ERA_allsite_proc$all_cpt_summ)
writeData(wb_out, "MOD_EVAL_SUMMARY", ERAvsOBS_eval$mod_eval_summary)
writeData(wb_out, "OBS_SEG_SUMMARY",  OBS_allsite_proc$all_seg_summ)
writeData(wb_out, "MOD_SEG_SUMMARY",  ERA_allsite_proc$all_seg_summ)
writeData(wb_out, "MOD_SUMSTATS",     ERAvsOBS_eval$mod_sumstats_main)
#Save the workbook.
